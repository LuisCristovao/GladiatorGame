  ...
  apply_rules = {
    ...
    ice: () => {
      ...
      character.frozen = Math.min(
        character.frozen + (attack_choices[water] + attack_choices[wind]) * 0.1,
        1.0
      );
    },
    //water+fire+earth
    ...
    vapor: () => {
      ...
      character.blind = Math.min(
        character.blind + (attack_choices[water] + attack_choices[fire]) * 0.1,
        1.0
      );
      character.attack = Math.max(character.attack - attack_choices[water], 1);
    },
    ...
    blue_fire: () => {
      ...
      character.burned = Math.min(
        character.burned + (attack_choices[fire] + attack_choices[wind]) * 0.1,
        1.0
      );
    },
    //fire + wind + earth
    ...
    ice_vapor: () => {
      character.health -=
        attack_choices[water] + attack_choices[fire] + attack_choices[wind];
      character.blind = Math.min(
        character.blind + (attack_choices[water] + attack_choices[fire]) * 0.1,
        1.0
      );
